...
from Helper import decode_ascii, decode_utf8
from pprint import pprint


def clearTags(htmltext, tagNames = ['pre', 'h2', 'h3','h4','table','span']):
...
def traverseHtml(titles, htmltext, state =1):
            ...
            if re.match(r'(\w\.)+', head): # head has to start with number, such as 3.2.1
                ...
                for grandchild in child.children:
                    if grandchild.name=="div" and "body" in grandchild['class']:
                        body += " " +getText(clearTags(grandchild))
                item["body"] = body
                titles.append(item)
                traverseHtml(titles, child, state=state + 1)
