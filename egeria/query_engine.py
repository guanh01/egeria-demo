...
import copy
import os
from egeria.helper import load_pickle, sents2tokens, stemmer 
...
class QueryEngineHtml(object):
    ...
    def __init__(self, model_dir, template_dir, docname):

        self.themodel = load_pickle(model_dir, settings.MODEL_FILE)
        self.dictionary = load_pickle(model_dir, settings.DICTIONARY_FILE)  
        self.simIndex = load_pickle(model_dir, settings.INDEX_FILE)
        self.docname = docname 

        self.soup = BeautifulSoup(open(os.path.join(template_dir, settings.SUMMARY_HTML)), "lxml")
        ...
        # print "Website is running for", self.docname 
...
#            
    ...
    def performQuery(self, issues, sim_thr):
        ...
        #duration = time.time()
        indexes = self.getRetrievedSentsIndex(self.simIndex, issues, sim_thr)
